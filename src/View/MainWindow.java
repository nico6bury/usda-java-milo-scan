...
public class MainWindow extends javax.swing.JFrame implements DisplayTaskCaller, IJTaskCaller {
	 ...
	 * The string formatting for number formatting and blank lines is handled in CustomRenderer.java.
	...
	public void updateOutputTable(List<List<SumResult>> groupedResults) {
        ...
        int lines_since_blank = 0;
        Object[] blank_row = new Object[] {"",-3,-3,-3,-3};
		for (List<SumResult> resultGroup : groupedResults) {
			for (SumResult res : resultGroup) {
                if (lines_since_blank >= 2) {
                    this_table_model.addRow(blank_row);
                    lines_since_blank = 0;
                }
            ...
            // add two blank lines, reset lines_since_blank
            this_table_model.addRow(blank_row);
            this_table_model.addRow(blank_row);
            lines_since_blank = 0;
    ...
    private void uxImageDpiMenuBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_uxImageDpiMenuBtnActionPerformed
		...
		JOptionPane.showMessageDialog(this, "Please be aware that changing this setting can break image processing. This is because a number of image processing settings are dpi-dependent.\n" +
            "Look at the comment above the dpi setting in the config file for more information on what to change if you really want to adjust dpi.");
